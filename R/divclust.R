...
divclust <- function (data, K = NULL, w = NULL)
{
  cl <- match.call()
  if (is.null(data)) {
  ...
  if(is.null(w)) w <- rep(1. / n, n) # default to equal weights if no user weights given
  dec <- gsvd(X_centre, c(rep(1, nb_quanti), 1./mu_quali), w)
